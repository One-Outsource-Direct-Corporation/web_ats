...
export const AssessmentManagement = ({
  ...
  const getStepId = (step: PipelineStep): number | string => {
    return (step as PipelineStepInDb).id ?? (step as PipelineStepLocal).tempId;
  };

  const getAssessmentId = (assessment: AssessmentInDb | AssessmentLocal) => {
    return assessment.id ?? assessment.tempId;
  };
  ...
  const handleAddQuestion = (question: Questionnaire) => {
    ...
    const updatedPipelines = pipelineSteps.map((step) => {
      ...
      const selectedStepId = getStepId(selectedPipelineStep);
        ...
        const updatedAssessments = step.assessments.map((assessment) => {
          ...
          if (id === selectedAssessmentId) {
            const updatedQuestions = (assessment.questions || []).map((q) => {
              ...
              const updatedQId = getQuestionId(updatedQuestion);
                ...
                <Select
                  value={
                    selectedPipelineStep
                      ? String(getStepId(selectedPipelineStep))
                      : ""
                  }
                  onValueChange={(value) => {
