...
export const useEmailTemplatesQuery = (params?: { pageSize?: number; initialSearch?: string; debounceMs?: number }) => {
  ...
  const fetchTemplates = useCallback(
    async (pageNum: number, searchQuery: string, append: boolean) => {
      ...
      try {
        setLoading(true);
        const response = await emailTemplateService.listResponse({ page: pageNum, pageSize, search: searchQuery }, { httpClient: axiosPrivate, signal: controllerRef.current.signal });
        setTemplates((prev) => (append ? [...prev, ...response.results] : response.results));
  ...
  return { templates, search, loading, hasMore, totalCount, loadMore, handleSearch, refetch };
};
