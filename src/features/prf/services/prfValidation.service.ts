...
export const prfJobPostingSchema = z
  .object({
    ...
    department: z.number({ message: "This field may not be null." }).nullable(),
  ...
  .refine((data) => data.department !== null, {
    path: ["department"],
...
export const prfCategorySchema = z.enum(
  ["managerial", "supervisory", "rank_and_file"],
  {
    message: "This field may not be null.",
  },
);

export function validatePrfJobPostingWithZod(
  jobPosting: unknown,
): ValidationError {
  const parsed = prfJobPostingSchema.safeParse(jobPosting);
  if (parsed.success) {
    return {};
  }
...
export function validatePrfPipelineWithZod(
  pipeline: unknown[],
...
export function validatePrfNonNegotiableWithZod(
