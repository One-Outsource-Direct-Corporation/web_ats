...
import {formatTime} from "@/shared/utils/formatDate.ts";
...
export default function PRFSidebarPreview({step, formData}: PRFSidebarPreview) {
    const [showMore, setShowMore] = useState<boolean>(false);

    const selectedHardware = Object.entries(formData.prf_input.hardware_required).filter(([, selected]) => selected)
        .map(([hardware])=>hardware.charAt(0).toUpperCase() + hardware.slice(1));

    const selectedSoftware = Object.entries(formData.prf_input.software_required).filter(([, selected]) => selected)
